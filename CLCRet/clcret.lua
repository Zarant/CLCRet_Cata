...
local function CmdLinePrioZeal(args)
	clcret.db.profile.rotation.prioZeal = args
	clcret.RR_UpdateQueue()
end
...
function clcret:QUEST_LOG_UPDATE()
	...
	self:RegisterChatCommand("clcretlpz", CmdLinePrioZeal)
...
function clcret.CreatePPB()
	...
	AA = a
	a:SetFromAlpha(0) a:SetToAlpha(1) a:SetDuration(0.5) a:SetOrder(1)
	a = myppb.glow.pulse:CreateAnimation("Alpha")
	a:SetFromAlpha(1) a:SetToAlpha(0) a:SetStartDelay(0.3) a:SetDuration(0.6) a:SetOrder(2)
	...
	a:SetFromAlpha(0) a:SetToAlpha(1) a:SetDuration(0.2) a:SetOrder(1)
	...
	a:SetFromAlpha(1) a:SetToAlpha(0) a:SetDuration(0.3) a:SetOrder(1)
